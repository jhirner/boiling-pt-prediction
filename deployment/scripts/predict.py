	...
	def check_warnings(self):
		...
		if self.smiles_trans.h_bond_donors > 1:
			self.warnings.append(("h_bond_donors",
				                  "Molecules with &ge;2 hydrogen bonding donors are not well represented in the training data set; the predicted boiling point is likely artificially low."))

		if self.smiles_trans.mol_wt > 308 or self.smiles_trans.mol_wt < 113:
			self.warnings.append(("mol_wt",
								  "The molecular weight is more than 2 standard deviations from the training data's mean; boiling points are not accurately predicted for molecules of this size."))
			
		if self.smiles_trans.rings_aromatic > 1:
			self.warnings.append(("rings_aromatic",
								  "Molecules with &ge;2 aromatic rings are not well represented in the training data set; boiling points are not accurately predicted for polyaromatics."))
